...
class PcapLoggerTest(unittest.TestCase):
    ...
    def test_log_with_filter(self):
        ...
        _filter = create_object('filter.portfilter.TCPFilter')
        _filter = apply_attrs(_filter, fcfg)
    ...
    def test_log_with_filter_inverse(self):
        config={'name':'PCAPLogger.TEST',
                'target':'/tmp/test.pcap',
                }
        logger = apply_attrs(self.logger, config)
        ...
        fcfg = { 'name':'TCPFilter',
                 'sport' : '82',    # 0 byte PCAP raises NeedData from dpkt
                 'inverse' : 'true'
                }
        _filter = create_object('filter.portfilter.TCPFilter')
        _filter = apply_attrs(_filter, fcfg)
